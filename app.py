...
BASIC_USER = (os.environ.get("BASIC_AUTH_USER") or "admin").strip() or "admin"
BASIC_PASS = (os.environ.get("BASIC_AUTH_PASS") or "change_me_please").strip() or "change_me_please"
...
def home(request: Request, _=Depends(require_admin_auth)):
    ...
    return templates.TemplateResponse(
        "home.html",
        {
            ...
            "sources": sources,
            "sub_token": SUB_TOKEN,
            "security_warn": (BASIC_PASS == "change_me_please") or (not SUB_TOKEN),
...
async def v2ray_to_clash_web(
    ...
    _ = (out or "yaml").strip().lower()
    _ = (template or "play").strip().lower()
...
async def source_save(
    ...
    _ = (clash_template or "play").strip().lower()
    clash_template = "play"
    rate_limit = _safe_int(rate_limit_mbps, 0)
